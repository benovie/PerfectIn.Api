<?php
namespace PerfectIn\Api\Aspect;
...
class LoggingAspect {
	 ...
	 * Logs calls
	 *
	 * @Flow\Before("method(PerfectIn\Api\Webservice\WebserviceCall->invoke())")
	 * @param \TYPO3\Flow\Aop\JoinPointInterface $joinPoint The current joinpoint
	 */
	public function logStartServiceCall(\TYPO3\Flow\Aop\JoinPointInterface $joinPoint) {
		$arguments = array();
		foreach ($joinPoint->getProxy()->getParameters() AS $arg) {
			$arguments[] = $this->getLogMessageForVariable($arg);
		}
		$this->logIdentifier = uniqid();
		$message = $this->logIdentifier. ' - request - ' .$joinPoint->getProxy()->getClass().'::'.$joinPoint->getProxy()->getMethod() . '(' . implode(', ',$arguments) .')'; 
